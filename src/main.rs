...
mod index;
use index::{ Program };
use index::engine::systems::{ EventSystem, InputSystem, DesktopInputHandler, InterfaceSystem };
                ...
                if locked {
                    ...
                    slint_window.with_winit_window(|winit_window| {
                        ...
                        if delta_x.abs() > 2.0 || delta_y.abs() > 2.0 {
                            ...
                            let camera_event = crate::index::engine::systems::event_system::Event {
                                event_type: crate::index::engine::systems::event_system::EventType::RotateCamera,
                                payload: Box::new(euler_deltas),
                            };
                            crate::index::engine::systems::event_system::EventSystem::notify(
                                camera_event
                            );
                        }
                    ...
                    InputSystem::instance().receive_mouse_event_owned(Box::new(event.clone()));
                    ...
                    match code {
                        ...
                        KeyCode::Tab => {
                            if keyboard_event.state == ElementState::Pressed {
                                ...
                                InputSystem::instance().receive_key_event_owned(Box::new(keyboard_event.clone()));
                                ...
                                slint_window.with_winit_window(|winit_window| {
                                    if !was_locked && is_locked {
                                        ...
                                        if
                                            let Err(e) =
                                                winit_window.set_cursor_position(center_pos)
                                        {
                ...
                if desktop_handler_events.borrow().is_cursor_locked() {
                    slint_window.with_winit_window(|winit_window| {
                        ...
                        if let Err(e) = winit_window.set_cursor_position(center_pos) {
                            ...
                            println!(
                                "[FPS] Cursor recentered after window resize to ({:.0}, {:.0})",
                                (new_size.width as f64) / 2.0,
                                (new_size.height as f64) / 2.0
                            );
                        }
